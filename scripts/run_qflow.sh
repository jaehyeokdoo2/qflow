...
for task_idx in $(seq $start_task_idx $end_task_idx); do
    echo "Running QFLOW PIPELINE for task ${task_idx}"
    echo "=========================================="
    echo ""
    ...
    # stitch datasets
    # env_name="antmaze-medium-stitch-singletask-task${task_idx}-v0"
    ...
    for seed in $seeds; do
        ...
        # If offline_steps is 2M, add "scale" suffix
        ...
        if [ "$override_params" = "True" ]; then
            # Override params: pass time_embed_dim and alpha via command line
            python main.py \
                --seed=$seed \
            ...
            # Use qflow_config.py values (don't pass time_embed_dim and alpha)
            python main.py \
                --seed=$seed \
                --eval_interval=$eval_interval \
                --offline_steps=$offline_steps \
