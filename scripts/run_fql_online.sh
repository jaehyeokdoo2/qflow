...
# Base directory where offline models were saved (must match base_dir in run_fql.sh)
restore_base_dir="fql-reproduce"

for task_idx in $(seq $start_task_idx $end_task_idx); do
    echo "Running FQL ONLINE FINE-TUNING for task ${task_idx}"
    echo "=========================================="
    echo ""

    # Environment settings - must match run_fql.sh
    ...
    restore_epoch=1000000  # Must match offline_steps in run_fql.sh

    # Hyperparameters - must match run_fql.sh
    flow_steps=10
    batch_size=256
    layer_width=256
    alpha=10
    ...
    for seed in $seeds; do
        ...
        offline_exp_name="fql_${env_name}_sd${seed}"
        restore_path="${restore_base_dir}/fql/${run_group}/${offline_exp_name}"

        echo "=========================================="
        echo "FQL ONLINE FINE-TUNING"
        echo "=========================================="
        echo "Task: ${task_idx}"
        echo "Seed: ${seed}"
