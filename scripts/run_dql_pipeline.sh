...
for seed in $(seq $start_seed $end_seed); do
    ...
    exp_name="${env_name}_alpha${alpha}-diff${diffusion_steps}-qagg${q_agg}-discount${reward_discount}_sd$(printf '%03d' $seed)"

    # Directory structure
    base_dir="exp"
    run_group=${env_name}
    # Final path will be: base_dir/dql/run_group/exp_name
    final_save_dir="${base_dir}/dql/${run_group}/${exp_name}"
