...
guidance_coeffs="1.5, 2.0, 2.5, 3.0, 5.0"
eval_episodes=50
save_summary=false
save_values=false
...
for seed in $(seq $start_seed $end_seed); do
    ...
    exp_name="${env_name}_expectile${expectile}-numsamples${num_samples}-flow${flow_steps}-discount${reward_discount}_sd$(printf '%03d' $seed)"

    # Directory structure
    base_dir="exp"
    run_group=${env_name}
    # Final path will be: base_dir/ifql/run_group/exp_name
    final_save_dir="${base_dir}/ifql/${run_group}/${exp_name}"
    ...
    mkdir -p "$eval_dir"
    for partial_guidance in 1 2 3 5 7; do
    
    python guidance_evaluation.py \
        --env_name="$env_name" \
        --agent_config="agents/ifql.py" \
