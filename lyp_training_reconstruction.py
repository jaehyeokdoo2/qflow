...
from utils.networks import EncoderDecoderLyapunovFunction
...
flags.DEFINE_list('encoder_hidden_dims', [64, 64], 'Encoder hidden layer dimensions.')
flags.DEFINE_list('decoder_hidden_dims', [64, 64], 'Decoder hidden layer dimensions.')
...
flags.DEFINE_boolean('module_layer_norm', False, 'Whether to use module layer normalization.')
...
flags.DEFINE_float('reconstruction_coeff', 1.0, 'Coefficient for reconstruction loss.')
...
def combined_loss(params, lyapunov_fn, batch, delta=0.1, noise_std=0.1, reconstruction_coeff=1.0, rng_key=None):
    """
    Compute combined loss: Lyapunov loss + Reconstruction loss
    
    Lyapunov loss: Expert actions should have HIGHER V(s,a) than perturbed actions
    L_V = E[max(0, V(s,a_perturbed) - V(s,a_expert) + δ)]
    
    Reconstruction loss: MSE between original and reconstructed (s,a) pairs
    L_R = E[||(s,a) - decode(encode(s,a))||²]
    
    Total loss: L = L_V + λ * L_R
    
    Args:
        params: Network parameters
        lyapunov_fn: Network instance (EncoderDecoderLyapunovFunction)
        batch: Batch of data with 'observations' and 'actions'
        delta: Margin for the Lyapunov loss function
        noise_std: Standard deviation of Gaussian noise for perturbed actions
        reconstruction_coeff: Coefficient for reconstruction loss
    ...
    v_expert = lyapunov_fn.apply(params, states, actions)
    ...
    v_perturbed = lyapunov_fn.apply(params, states, perturbed_actions)
    ...
    # Compute reconstruction loss separately using encoder and decoder
    latent = lyapunov_fn.apply(params, states, actions, method=lyapunov_fn.encode)
    decoded_states, decoded_actions = lyapunov_fn.apply(params, latent, method=lyapunov_fn.decode)
    
    # Compute MSE loss
    state_loss = jnp.mean((states - decoded_states) ** 2)
    action_loss = jnp.mean((actions - decoded_actions) ** 2)
    ...
    # Total loss
    total_loss = lyapunov_loss + reconstruction_coeff * reconstruction_loss
...
def train_step(state, lyapunov_fn, batch, rng_key, delta=0.1, noise_std=0.1, reconstruction_coeff=1.0):
    """Perform a single training step with both Lyapunov and reconstruction loss."""
    def loss_fn(params):
        return combined_loss(params, lyapunov_fn, batch, delta, noise_std, reconstruction_coeff, rng_key)
...
def main(_):
    ...
    encoder_hidden_dims = [int(x) for x in FLAGS.encoder_hidden_dims]
    decoder_hidden_dims = [int(x) for x in FLAGS.decoder_hidden_dims]
    lyapunov_fn = EncoderDecoderLyapunovFunction(
        ...
        module_layer_norm=FLAGS.module_layer_norm
    )
    ...
    print(f"  Encoder: {state_dim + action_dim} -> {encoder_hidden_dims} -> {FLAGS.latent_dim}")
    print(f"  Decoder: {FLAGS.latent_dim} -> {decoder_hidden_dims} -> {state_dim + action_dim}")
    print(f"  Lyapunov: {FLAGS.latent_dim} -> {hidden_dims} -> 1")
    print(f"Lyapunov loss hyperparameters:")
    print(f"  Delta (margin): {FLAGS.delta}")
    print(f"  Noise std: {FLAGS.noise_std}")
    print(f"  Reconstruction coefficient: {FLAGS.reconstruction_coeff}")
    print(f"  Learning rate: {FLAGS.learning_rate}")
    print(f"  Batch size: {FLAGS.batch_size}")
    print(f"  Layer normalization: {FLAGS.layer_norm}")
    print(f"  Module layer normalization: {FLAGS.module_layer_norm}")
    ...
    for i in tqdm.tqdm(range(1, FLAGS.training_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        state, update_info = train_step(state, lyapunov_fn, batch, step_rng, FLAGS.delta, FLAGS.noise_std, FLAGS.reconstruction_coeff)
        ...
        if i % FLAGS.log_interval == 0:
            ...
            if val_dataset is not None:
                ...
                _, val_info = combined_loss(state.params, lyapunov_fn, val_batch, FLAGS.delta, FLAGS.noise_std, FLAGS.reconstruction_coeff, val_rng)
