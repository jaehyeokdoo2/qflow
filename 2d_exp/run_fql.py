"""
Run FQL (Flow Q-Learning) toy 2D experiment.

Usage:
    python run_fql.py --dataset two_spirals --alphas 0.5 0.3
"""
import argparse
import os
import sys
...
from models import onestep_forward, integrate_flow
from train.fql import train_fql


def parse_args():
    p = argparse.ArgumentParser(description="FQL 2D Toy Experiment")
...
def main():
    ...
    plot_dataset(x_1_data, rewards_data, args.dataset, args.save_dir)

    results = []
    for alpha in args.alphas:
        print(f"\n--- FQL alpha={alpha} ---")
        flow_params, policy_params, critic_params, bc_policy_params, history = train_fql(
        ...
        results.append((alpha, policy_params, bc_policy_params))
    ...
    # BC baseline
    bc_policy_params = results_sorted[0][2]
    ...
    for i, (alpha, policy_params, _) in enumerate(results_sorted):
    ...
    plt.tight_layout()
    out_dir = f'{args.save_dir}/{args.dataset}/fql'
