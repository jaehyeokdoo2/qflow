"""
Run QFlow toy 2D experiment.

Usage:
    python run_qflow.py --dataset two_spirals --alphas 0.5 0.3
"""
import argparse
import os
import sys
...
from train.qflow import train_qflow


def parse_args():
    p = argparse.ArgumentParser(description="QFlow 2D Toy Experiment")
    ...
    p.add_argument('--use_time_embed', action='store_true')
...
def main():
    ...
    for alpha in args.alphas:
        print(f"\n--- QFlow alpha={alpha} ---")
        flow_params, inner_params, bc_policy_params, history = train_qflow(
    ...
    plt.tight_layout()
    out_dir = f'{args.save_dir}/{args.dataset}/qflow'
